...
export function initMessages(root = document) {
  root.querySelectorAll("[data-message-item]").forEach((item, index) => {
    ...
    window.setTimeout(() => revealMessage(item), index * 100);
  });
...
export function showMessage(message, type = "error") {
  ...
  window.setTimeout(() => revealMessage(item), 100);
}
  ...
  item.innerHTML = `
    <div class="flex items-start">
      ...
      <div class="ml-3 flex-shrink-0">
        <button data-message-dismiss type="button" class="-m-3 inline-flex h-11 w-11 items-center justify-center rounded-md ${isError ? "text-red-600 hover:text-red-800 focus:ring-red-500" : "text-green-600 hover:text-green-800 focus:ring-green-500"} focus:outline-none focus:ring-2 focus:ring-offset-2">
...
function startAutoDismiss(item) {
  if (prefersReducedMotion()) {
    window.setTimeout(() => hideMessage(item), MESSAGE_DURATION_MS);
    return;
  }
